...
IMPORTANT: Do NOT use any markdown formatting like **bold** (double asterisks) or *italics*. Return clean, plain text only.
 ...
 * GET JDS BY EMPLOYER
 * 
 * Retrieves all job descriptions generated by a specific employer.
 * Used for the "Saved JDs" dashboard.
 * 
 * @async
 * @param {Object} req - Express request with employerId in params
 * @param {Object} res - Express response object
 * @returns {Array} List of job description documents
 */
export const getJDsByEmployer = async (req, res) => {
  try {
    ...
    // Find all JDs for this employer, sorted by most recent first
    const jds = await JobDescription.find({ employerId })
      .sort({ createdAt: -1 });
      
    res.status(200).json(jds);
  } catch (err) {
    console.error("Error fetching JDs by employer:", err);
    res.status(500).json({ message: "Failed to fetch saved JDs", error: err.message });
  }
};
